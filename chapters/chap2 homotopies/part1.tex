...
\begin{proposition}
La relation d'homotopie entre chemins est une relation d'équivalence. Nous noterons~$[\gamma]$ la classe d'équivalence de $\gamma$ sous la relation d'homotopie, appellée \emph{classe d'homotopie} de $\gamma$.
\end{proposition}
...
\bigskip \textit{(Inverse)} Soit $\alpha\in\pi_1(X,x_0)$ et soit $\gamma$ un lacet représentant de la classe. En rappelant que $\overline{\gamma}$ désigne le lacet en sens inverse de $\gamma$, montrons que $\gamma\cdot\overline{\gamma}\simeq c$ et $\overline{\gamma}\cdot\gamma\simeq c$, ce qui suffit pour démontrer l'existance de l'inverse : $\alpha\cdot [\overline{\gamma}]=[c]$ et $[\overline{\gamma}]\cdot \alpha=[c]$.
...
De ce résultat en découle l'application bien définie~$\Phi:\pi_1(X,x_0)\to\pi_1(X,x_1)$, qui envoie~$[\gamma]$ sur $[\zeta\cdot\gamma\cdot\overline{\zeta}]$. Pour montrer qu'il s'agit d'un isomorphisme, nous allons tout d'abord montrer qu'il s'agit d'un morphisme de groupes, puis qu'il admet une réciproque.
...
\textit{(Réciproque)} Considérons l'application $\Psi:\pi_1(X,x_1)\to\pi_1(X,x_0)$ définit par~$\Psi([\gamma])=[\overline{\zeta}\cdot\gamma\cdot\zeta]$. Pour la même raison que pour $\Phi$, il s'agit d'un morphisme de groupes bien définit. Il est évident que $\Phi$ et $\Psi$ sont des morphismes réciproques l'un par rapport à l'autre. 
...
\begin{definition}
Soit $f:(X,x_0)\to(Y,y_0)$ une application entre deux espaces, avec $f(x_0)=y_0$. Le \emph{morphisme de groupes induit} par $f$ est définit comme étant le morphisme $f_\ast:\pi_1(X,x_0)\to\pi_1(Y,y_0)$, envoyant $[\gamma]\in\pi_1(X,x_0)$ sur~$[f\gamma]\in\pi_1(Y,y_0)$. 
\end{definition}
...
Un foncteur covariant est un outil puissant, car il permet de converser les propriétés des fonctions (appelés flèches) et de les translater d'une catégorie à l'autre.
...
Attention, la réciproque n'est pas vrai, comme le montre les résultats suivants. 
...
L'idée de la démonstration est identique à celle préédente, à l'exception près qu'il ne s'agit pas d'égalités entre les compositions d'applications, mais d'équivalence d'homotopies. Cela demande légèrement plus de travail, et un résultat sur les homotopies et leurs morphismes induits. Nous avons décider de ne pas rentrer dans les détails ici, mais la démonstration se trouve en \cite{Hatcher} page 37.
