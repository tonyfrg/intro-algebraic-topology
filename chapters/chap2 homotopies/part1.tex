...
\caption{Lacet "coincé" dans le tore}
...
Ce chapitre est décomposé en trois grandes sections : \begin{enumerate}
    ...
    \item Le groupe fondamental du cercle. Sa caractérisation n'est pas si triviale qu'il n'y paraît, et implique notamment l'introduction des revêtements. Suite à ce résultat, nous arrivons à caractériser plusieurs autres groupes fondamentaux.
...
\end{tikzpicture}
\caption{Ruban de Möbius et deux lacets}
\label{tkz:mobius-loops}
\end{subfigure}
\end{figure}

Nous pouvons tracer une infinité de lacets sur le ruban, mais deux grandes "familles" en ressortent, illustrées dans la figure \ref{tkz:mobius-loops} : les lacets qui restent dans l'intérieur du carré (comme le lacet $\gamma$), et les lacets qui utilisent l'identification $A$ des côtés (comme le lacet $\gamma'$).
\end{exemple}
...
Soient $x_0, x_1$ deux points de l'espace $X$, et soient $\gamma_0,\gamma_1$ deux chemins partageant les mêmes extrémités $x_0$ et $x_1$. Une \emph{homotopie} entre $\gamma_0$ et $\gamma_1$ est une application $\Gamma:[0,1]^2\to X$, définie par~$\Gamma(t,s)=\gamma_t(s)$, continue en $t$ et telle que, pour tout $t\in[0,1]$, $\gamma_t(0)=x_0$ et $\gamma_t(1)=x_1$.

Intuitivement, cela revient à dire qu'il existe une famille $\gamma_t$ de chemins fixant les extrémités, et continue en $t$. Nous disons alors que $\gamma_0$ est homotope à $\gamma_1$, ou bien que $\gamma_0$ et $\gamma_1$ sont homotopes, et l'on note $\gamma_0\simeq\gamma_1$.
\end{definition}
...
\begin{proposition}
La relation d'homotopie entre chemins est une relation d'équivalence. Nous noterons~$[\gamma]$ la classe d'équivalence de $\gamma$ sous la relation d'homotopie, appelée \emph{classe d'homotopie} de $\gamma$.
\end{proposition}
...
\begin{proof}
\textit{(Réflexivité)} Pour tout chemin $\gamma$ sur $X$, il existe l'homotopie constante $\gamma_t(s)=\gamma(s)$. Nous en concluons que $\gamma\simeq\gamma$.
...
%En effet, la concaténation de deux lacets donne un lacet qui passera sur le point de base à mi-parcours. Nous pouvons visualiser cela avec le chiffre 8, qui est concaténation d'un lacet supérieur et un lacet inférieur, où le point de base est le point d'intersection des deux lacets.
...
\begin{proof}
Soient $\gamma\simeq\gamma'$ et $\zeta\simeq\zeta'$ quatre lacets ayant un même point de base $x_0\in X$. Il nous suffira de montrer que $\gamma\cdot\zeta\simeq\gamma'\cdot\zeta'$.
...
Montrons que $\tau_t$ est une homotopie entre $\gamma\cdot\zeta$ et $\gamma'\cdot\zeta'$. D'une part, les points de départs et d'arrivées sont respectivement $\tau_0=\gamma_0\cdot\zeta_0=\gamma\cdot\zeta$, et $\tau_1=\gamma_1\cdot\zeta_1=\gamma'\cdot\zeta'$. Pour ce qui est des extrémités des lacets, ceux-ci restent constants en $x_0$ par définition de $\gamma_t$ et $\zeta_t$ : $\gamma_t\cdot\zeta_t(0)=\gamma_t(0)=x_0$ et $\gamma_t\cdot\zeta_t(1)=\zeta_t(1)=x_0$. Enfin la continuité de $\tau_t$ est évidemment due à celle des deux autres homotopies.
...
On construit tout d'abord l'application permettant d'"effacer" homotopiquement le lacet constant de~$\gamma\cdot c$ : \[t\in[0,1]\mapsto\gamma_t(s)=\left\{\begin{matrix}
...
De manière similaire, nous pouvons construire l'application permettant d'effacer homotopiquement le lacet constant de $c\cdot\gamma$ : \[t\in[0,1]\mapsto\gamma'_t(s)=\left\{\begin{matrix}
c\left(0\right)=x_0&\text{si }0\leq s\leq \frac{1}{2}(1-t)\\ 
\gamma\left(\frac{2s+t-1}{t+1}\right)&\text{si }\frac{1}{2}(1-t)\leq s\leq 1.
\end{matrix}\right.\]

Montrons qu'il s'agit d'une homotopie, en commençant par la continuité. Pour $s=\frac{1}{2}(1-t)$, nous avons l'égalité ${2s+t-1=(1-t)+t-1=0}$, d'où $\gamma\big(\frac{2s+t-1}{t+1}\big)=x_0$. Cela montre que notre application est continue. De même que précédemment, il est facile de vérifier que cela constitue une homotopie entre les lacets~$c\cdot\gamma$ et $\gamma$.
...
\bigskip \textit{(Inverse)} Soit $\alpha\in\pi_1(X,x_0)$ et soit $\gamma$ un lacet représentant de la classe. En rappelant que $\overline{\gamma}$ désigne le lacet en sens inverse de $\gamma$, montrons que $\gamma\cdot\overline{\gamma}\simeq c$ et $\overline{\gamma}\cdot\gamma\simeq c$, ce qui suffit pour démontrer l'existence de l'inverse : $\alpha\cdot [\overline{\gamma}]=[c]$ et $[\overline{\gamma}]\cdot \alpha=[c]$.
...
\end{matrix}\right.\]Montrons qu'il s'agit d'une homotopie, en commençant par la continuité. La continuité en $t$ est évidente car il ne s'agit que de chemin linéaire sur $[0,1]$. La continuité en $s$ est tout également simple, mais vérifions la jonction en $1\over2$ : d'un côté, nous avons $\gamma(2s(1-t))=\gamma(1-t)$, et de l'autre $\overline{\gamma}((2s-1)(1-t)+t)=\overline{\gamma}(t)=\gamma(1-t)$. Aux extrémités de $s$, nous avons $\gamma_t(0)=\gamma(0)=x_0$, et $\gamma_t(1)=\overline{\gamma}(1-t+t)=\gamma(0)=x_0$. Il s'agit finalement d'une homotopie entre $\gamma_0=\gamma\cdot \overline{\gamma}$ et $\gamma_1=\gamma(0)\cdot\overline{\gamma}(0)=c$. %Ensuite, il est facile de vérifier la continuité en $t$, et que l'on a $\gamma_0=\gamma\cdot\overline{\gamma}$ et $\gamma_1=c$. 
Nous pouvons alors en déduire que cette application est une homotopie entre les lacets $\gamma\cdot\overline{\gamma}$ et $c$.
...
Dans un espace convexe, il existe une homotopie linéaire (voir l'exemple qui suit la définition \ref{def:homotopy-paths}) entre toute paire de lacet. De ce fait, tous les lacets sont homotopes entre eux : d'où la proposition.

\begin{proposition}
Soient $x_0,x_1\in X$ deux points reliés via un chemin. Les groupes fondamentaux $\pi_1(X,x_0)$ et $\pi_1(X,x_1)$ sont isomorphes.
\end{proposition}
...
\begin{proof}
Soit $\zeta$ un chemin reliant $x_0=\zeta(0)$ et $x_1=\zeta(1)$. Pour un lacet~$\gamma$ ayant pour point de base~$x_0$, la concaténation $\zeta\cdot\gamma\cdot\overline{\zeta}$ est un lacet basé en~$x_1$. L'idée de la démonstration est de construire un isomorphisme se basant sur cette idée, tout en utilisant les classes d'homotopies.

Remarquons avant toute chose que pour deux lacets $\gamma\simeq\gamma'$ de $(X,x_0)$, les lacets $\zeta\cdot\gamma\cdot\overline{\zeta}$ et $\zeta\cdot\gamma'\cdot\overline{\zeta}$ sont eux aussi homotopes sur $(X,x_1)$. En effet, en considèrant $\gamma_t$ l'homotopie entre $\gamma$ et $\gamma'$, nous obtenons directement l'homotopie $\zeta\cdot\gamma_t\cdot\overline{\zeta}$ entre les lacets $\zeta\cdot\gamma\cdot\overline{\zeta}$ et $\zeta\cdot\gamma'\cdot\overline{\zeta}$.

Ce résultat implique que l'application~$\Phi:\pi_1(X,x_0)\to\pi_1(X,x_1)$, envoyant~$[\gamma]$ sur $[\zeta\cdot\gamma\cdot\overline{\zeta}]$, est bien définie. Pour montrer qu'il s'agit d'un isomorphisme, nous allons tout d'abord montrer qu'il s'agit d'un morphisme de groupes, puis qu'il admet une réciproque.

\bigskip \textit{(Morphisme)} Soient $\alpha,\alpha'\in\pi_1(X,x_0)$, montrons que $\Phi(\alpha\alpha')=\Phi(\alpha)\Phi(\alpha')$. Pour cela, on considère $\gamma,\gamma'$ deux lacets de $(X,x_0)$ représentants respectifs de $\alpha$ et $\alpha'$ : \[\begin{split}
...
\textit{(Réciproque)} Considérons l'application $\Psi:\pi_1(X,x_1)\to\pi_1(X,x_0)$ définie par~$\Psi([\gamma])=[\overline{\zeta}\cdot\gamma\cdot\zeta]$. Pour la même raison que pour $\Phi$, il s'agit d'un morphisme de groupes bien défini. Il est évident que $\Phi$ et $\Psi$ sont des morphismes réciproques l'un par rapport à l'autre. 

Nous pouvons en conclure que les deux groupes sont isomorphes.
\end{proof}

\begin{remark}
Il en découle que le groupe fondamental d'un espace connexe par arc ne dépend pas du point choisi. Nous pouvons, sans perte de rigueur, négliger le point de base lors de la désignation du groupe fondamental.
\end{remark}
...
\begin{definition}
Soit $f:(X,x_0)\to(Y,y_0)$ une application entre deux espaces, avec $f(x_0)=y_0$. Le \emph{morphisme de groupes induit} par $f$ est défini comme étant le morphisme $f_\ast:\pi_1(X,x_0)\to\pi_1(Y,y_0)$, envoyant~$[\gamma]\in\pi_1(X,x_0)$ sur~$[f\gamma]\in\pi_1(Y,y_0)$. 
\end{definition}

En théorie des catégories (voir annexe \ref{chap:categories}), on dit que le groupe fondamental est un \emph{foncteur} entre la catégorie des espaces pointés et la catégorie des groupes. Intuitivement, cela revient à dire qu'il est possible de faire un parallèle entre les relations sur les espaces pointés et leurs groupes fondamentaux.
...
\end{theorem}
\begin{proof}
Premièrement, le morphisme induit par l'identité est défini par $id_\ast:[\gamma]\mapsto[id(\gamma)]=[\gamma]$ : il s'agit du morphisme identité.

Deuxièmement, pour $f:X\to Y$ et $g:Y\to Z$, nous avons : \[f_\ast(g_\ast([\gamma]))=f_\ast([g\gamma])=[fg\gamma]=(fg)_\ast([\gamma]).\]
\end{proof}

Un foncteur covariant est un outil puissant, car il permet de conserver les propriétés des fonctions (appelées flèches) et de les translater d'une catégorie à l'autre.

\begin{theorem}
Soient $X$ et $Y$ deux espaces homéomorphes par $f:X\to Y$, et soit $x_0\in X$. Le morphisme induit $f_\ast:\pi_1(X,x_0)\to\pi_1(Y,f(x_0))$ est un isomorphisme.
\end{theorem}
\begin{proof}
Du fait que $f$ soit une bijection, elle admet une inverse. Cette inverse induit une inverse pour $f_\ast$. Il en découle alors que $f_\ast$ soit un isomorphisme.
\end{proof}
...
\begin{corollary}
Si deux espaces $(X,x_0)$ et $(Y,y_0)$ ne possèdent pas le même groupe fondamental, alors ils ne sont pas homéomorphes.
\end{corollary}

Attention, la réciproque n'est pas vraie, comme le montre les résultats suivants. 

\begin{proposition}
Si un espace $X$ se rétracte en $A$, alors le morphisme $i_\ast:\pi_1(A,x_0)\to\pi_1(X,x_0)$ induit par $i: A\hookrightarrow X$ est injectif. Si c'est une déformation par rétraction, alors le morphisme induit est un isomorphisme.
\end{proposition}
\begin{proof}
...
L'idée de la démonstration est identique à celle précédente, sauf qu'il ne s'agit pas d'égalités entre les compositions d'applications mais d'équivalence d'homotopies. Cela demande légèrement plus de travail, et un résultat sur les homotopies et leurs morphismes induits. Nous avons décidé de ne pas rentrer dans les détails ici, mais la démonstration se trouve en \cite{Hatcher} page 37.

\bigskip Ces derniers résultats imposent une limite dans la discrimination des espaces : s'il en existe deux qui se "ressemblent" assez sans être homéomorphes, il est possible que le groupe fondamental ne suffise pas pour les distinguer.